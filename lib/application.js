...
const saveInvoiceFileLocally = async (file) => {
  ...
  const isImage = file.type?.startsWith("image/") || file.name?.toLowerCase().match(/\.(jpg|jpeg|png|webp|heic|heif)$/);
  if (!isPdf && !isImage) {
    throw new Error("Only image or PDF uploads are supported.");
  }
