...
export default function AdminSidebar({ userRole, userName }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex flex-col items-center justify-center border-b border-gray-100/80 shrink-0 transition-all duration-500 ${isCollapsed ? "h-24 px-2" : "h-28 px-6"}`}
      >
        <Link
          href="/"
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto overflow-x-hidden custom-scrollbar">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div
                className={`relative flex items-center justify-center shrink-0 transition-transform ${isCollapsed && !isActive ? "group-hover:scale-110" : ""}`}
              >
                <Icon
                  size={22}
      ...
      <div
        className={`p-4 border-t border-gray-100 shrink-0 transition-all duration-500`}
      >
        <button
