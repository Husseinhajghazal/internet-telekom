              ...
              >
                ...
                <div className="px-5 pt-5 pb-3 relative z-10 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                  <div className="flex flex-col items-start justify-center">
                    <span className={`flex items-baseline justify-start gap-1 text-2xl font-black tracking-tighter bg-clip-text text-transparent bg-gradient-to-br ${palette.textGradient}`}>
                      <span className="text-base font-bold text-gray-500 uppercase">Mbps</span>
                      <span>{speed}</span>
                    </span>
                    <span className={`text-lg font-extrabold ${isSelected ? palette.priceText : "text-gray-800"}`}>
                      {displayPrice} <span className="text-[10px] text-gray-500 font-bold">TL / شهرياً</span>
                    </span>
                  </div>
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors duration-500 ${
                      isSelected ? palette.badgeBg + " text-white shadow-md shadow-" + theme + "-500/30" : palette.iconBg
                    }`}
                  >
